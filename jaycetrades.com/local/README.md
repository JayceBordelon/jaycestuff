# JayceTrades — Local Development Runtime

A self-contained Docker stack for testing JayceTrades locally with realistic seeded data. No production credentials, no external API calls, no Traefik.
...
- **Postgres**: `localhost:5433` (user `jaycetrades`, password `jaycetrades`, db `jaycetrades`)
...
```bash
psql postgresql://jaycetrades:jaycetrades@localhost:5433/jaycetrades
```

Or via Docker:

```bash
docker exec -it jt-local-postgres psql -U jaycetrades
```
...
- **API protection** — `curl http://localhost:8080/api/trades/today` returns 403 (missing `X-JT-Source`); the frontend includes the header automatically
